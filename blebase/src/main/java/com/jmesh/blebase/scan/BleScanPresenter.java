package com.jmesh.blebase.scan;
...
import com.jmesh.blebase.callback.BleScanPresenterImp;
...
public abstract class BleScanPresenter implements BluetoothAdapter.LeScanCallback {
    public static final String LogTag = BleScanPresenter.class.getSimpleName();
    ...
    private void correctDeviceAndNextStep(final BleDevice bleDevice) {
        if (mNeedConnect) {
            Log.i(LogTag, "devices detected  ------"
                    + "  name:" + bleDevice.getName()
                    + "  mac:" + bleDevice.getMac()
                    + "  Rssi:" + bleDevice.getRssi()
                    + "  scanRecord:" + HexUtils.formatHexString(bleDevice.getScanRecord()));
            ...
            for (BleDevice result : mBleDeviceList) {
                if (result.getDevice().equals(bleDevice.getDevice())) {
                    hasFound.set(true);
                    if (!Arrays.equals(result.getScanRecord(), bleDevice.getScanRecord())) {
                        result.setScanRecord(bleDevice.getScanRecord());
                        mMainHandler.post(new Runnable() {
                            @Override
                            public void run() {
            ...
            if (!hasFound.get()) {
                Log.i(LogTag, "device detected  ------"
                        + "  name: " + bleDevice.getName()
                        + "  mac: " + bleDevice.getMac()
                        + "  Rssi: " + bleDevice.getRssi()
                        + "  scanRecord: " + HexUtils.formatHexString(bleDevice.getScanRecord(), true));
