...
public class BleScanner {
    public static final String LogTag = BleScanner.class.getSimpleName();
    ...
    public void clearBleDeviceList() {
        if (mBleScanPresenter != null) {
            mBleScanPresenter.clearBleDeviceList();
        }
    }
    ...
    private synchronized void startLeScan(UUID[] serviceUuids, String[] names, String mac, boolean fuzzy,
        ...
        if (mBleScanState != BleScanState.STATE_IDLE) {
            Log.w(LogTag, "scan action already exists, complete the previous scan action first");
            if (imp != null) {
